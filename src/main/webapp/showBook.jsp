...
<c:forEach begin="1" end="${pageInfo.pages}" var="i"> <c:if test="${pageInfo.pageNum==i}"> <a
        href="/show? pageSize=${i}&pageNumber=3&name=${book.book_name}&type=${book.content}"
        style="color: red">${i}</a>&nbsp&nbsp </c:if> <c:if test="${pageInfo.pageNum!=i}"> <a
        href="/show? pageSize=${i}&pageNumber=3&name=${book.book_name}&type=${book.content}">${i} </a>&nbsp&nbsp </c:if>
</c:forEach>
